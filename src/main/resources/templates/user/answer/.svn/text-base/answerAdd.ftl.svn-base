...
<div class="panel-body">
    ...
    <div class="well">
        <form class="form-horizontal" action="${base}/user/answer/createAnswer" method="post" id="filter">
            <input type="hidden" value="" name="attachs" id="attachs" />

            <div class="form-group">
                <label for="uid" class="col-md-2 control-label">问题内容:</label>
                <div class="col-md-8">
                    <p>
                        【<strong><#if question.user.type==2 || question.user.type==3>${(question.user.expert_name)!''}<#else>${(question.user.userInfo.data.nick_name)!''}</#if></strong>的提问】${(question.content)!''}
        ...
        <#if (answer)??>
            <div class="form-group">
                ...
                <div class="col-md-8">
                    <p>
                        【<strong>${(answer.user.userInfo.data.nick_name)!''}</strong>的回复】${(answer.content)!''}
                        <input type="hidden" name="replyUid" value="${(answer.uid)!''}"/>
                        <input type="hidden" name="replyAnswerId" value="${(answer.answerId)!''}"/>
                    </p>
                </div>
            </div>
        </#if>
            ...
            <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                    <button type="button" class="btn btn-primary" id="but_create_answer">发布</button>
                </div>
            </div>
